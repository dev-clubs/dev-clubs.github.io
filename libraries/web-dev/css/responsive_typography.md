...
| **Note:** You may even need to consider the primary language of your user, as you may need to leave room for accents and other special characters. Learn more about this in the [internationalization](https://web.dev/learn/design/internationalization) module.
...
| **Note:** You can mitigate the jarring effect of text shifting around by using [`size-adjust`](https://web.dev/articles/css-size-adjust) in your `@font-face` rule.
...
| **Note:** If you're styling elements using the `system-ui` value for `font-face` property, you might get all the benefits of variable fonts without downloading any font files. [More and more system fonts are now variable fonts](https://web.dev/blog/more-variable-font-options-in-chromium-83).
...
False  
Text will wrap by default without any additional styles. `clamp()` is useful for fluid typography because  
...
Try again. Which type of `line-height` values were recommended in this guide?  
`24px`  
...
`2vw`  
Viewport units as `line-height` would problematic. What does `font-display` do?  
